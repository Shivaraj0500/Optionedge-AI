...
export default async function(req, res) {
  ...
  try {
    ...
    return res.json({ analysis, run_id: saved.rows[0].id, created_at: saved.rows[0].created_at, context_summary: {strategy_version:strategy?.version || null, campaign_status:campaign?.status || 'NONE', live_context_verified:Boolean(decision)} });
  } catch (error) {
