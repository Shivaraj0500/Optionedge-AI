...
export async function placeLiveOrder({userId,instrumentKey,transactionType,quantity,idempotencyKey,campaignId=null,legId=null}){
  ...
  const payload={quantity,product:String(s.cfg.product||'I'),validity:String(s.cfg.validity||'DAY'),price:0,tag:('OE_'+idempotencyKey).slice(0,40),instrument_token:instrumentKey,order_type:String(s.cfg.order_type||'MARKET'),transaction_type:transactionType,disclosed_quantity:0,trigger_price:0,is_amo:false,slice:!!s.cfg.auto_slice,market_protection:Number(s.cfg.market_protection??-1)};
