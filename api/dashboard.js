...
export default async function(req,res){
  ...
  try {
    ...
    const [nifty, banknifty, tradesQ]=await Promise.all([
      ...
      db.query(`SELECT l.id,l.campaign_id,l.strategy_id,l.strategy_version,l.role,l.side,l.option_type,l.execution_rank,l.expiry,l.strike,l.trading_symbol,l.instrument_key,l.quantity,l.entry_price,l.current_price,l.pnl,l.status,l.entry_at,l.last_mark_at,c.underlying,c.status AS campaign_status,c.started_at,c.mode,c.strategy_version_id,s.name AS strategy_name,s.signal_model,s.timeframe,s.candle_type,v.version_number AS pinned_version_number FROM paper_campaign_legs l JOIN paper_campaigns c ON c.id=l.campaign_id AND c.user_id=l.user_id LEFT JOIN strategy_configs s ON s.id=c.strategy_id AND s.user_id=c.user_id LEFT JOIN strategy_versions v ON v.id=c.strategy_version_id AND v.strategy_id=c.strategy_id AND v.user_id=c.user_id WHERE l.user_id=$1 AND l.status='OPEN' AND c.status='RUNNING' ORDER BY c.started_at DESC,l.execution_rank ASC`,[userId])
    ]);
